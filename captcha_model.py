...
#==========================train_data
traincsv = open('./number_train.csv', 'r', encoding = 'utf8')
train_data = np.stack([np.array(Image.open("D:/cbc106013/captcha_cnn/captcha/number_train/" + row[0] + ".jpg"))/255.0 for row in csv.reader(traincsv)])
...
for i in range(1,5,1):
    file = open('number_train.csv', 'r')  # csv_file_name
...
print(train_label)
#==========================test_data
testcsv = open('./number_test.csv', 'r', encoding = 'utf8')
test_data = np.stack([np.array(Image.open("D:/cbc106013/captcha_cnn/captcha/number_test/" + row[0] + ".jpg"))/255.0 for row in csv.reader(testcsv)])
...
for i in range(1,5,1):
    file = open('number_test.csv', 'r')  # csv_file_name
    reader = csv.reader(file)
    for row in reader:
        digit.append(row[i])
...
earlystop = EarlyStopping(monitor='val_digit4_acc', patience=20, verbose=1, mode='auto')
#===========================圖形化界面
